#!/usr/bin/env python3
"""
Trajectory Bridge for Mobile ALOHA Robot
Provides REST endpoints for trajectory-based motion control with safety validation
"""
...
# Try to import arm_controller, fallback to mock if not available
try:
    ...
    USE_MOCK = True
    class ArmController:
        """Mock arm controller when real one not available"""
...
logger = logging.getLogger('trajectory_bridge')
...
class TrajectoryBridge:
    ...
    def __init__(self, port=8082):
        ...
        # Configuration
        self.driver_mode = 'mock' if USE_MOCK else os.getenv('ROBOT_DRIVER', 'real')  # real or mock
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.arm_controller = ArmController(
                ...
                enable_safety=True,
                dry_run=(self.driver_mode == 'mock')
            )
            self.arm_controller.connect()
            logger.info(f"Initialized arm controller (mode: {self.driver_mode})")
            ...
            logger.info("Trajectory bridge initialized successfully")
    ...
    async def handle_follow_trajectory(self, request):
        ...
        try:
            ...
            if self.arm_controller and hasattr(self.arm_controller, 'current_position'):
                current_pos = self.arm_controller.current_position
            else:
                current_pos = [0.25, 0.0, 0.2]  # Default safe position
            end_pos = [
    ...
    async def handle_e_stop(self, request):
        ...
        try:
            # E-stop the arm controller
            if self.arm_controller:
                # Would implement actual e-stop here
                pass
    ...
    async def _queue_worker(self, arm_side: str):
        ...
        while True:
            try:
                ...
                if job.dry_run:
                    # Simulate execution
                    await self._execute_dry_run(job)
                else:
                    # Real execution
    ...
    async def _execute_job(self, job: Job):
        """Execute a real job on hardware"""
        try:
            if self.driver_mode == 'mock':
                # For mock mode, simulate execution
                await self._execute_dry_run(job)
                # Update mock position
                if job.type == "move_to_pose":
                    req = job.request
                    self.arm_controller.current_position = [req['x'], req['y'], req['z']]
                    job.result = {"success": True, "position": [req['x'], req['y'], req['z']]}
            else:
                # Real hardware execution
                if job.type == "move_to_pose":
                    req = job.request
                    result = self.arm_controller.move_to_position(
                        x=req['x'],
                        y=req['y'],
                        z=req['z'],
                ...
                elif job.type == "follow_cartesian_trajectory":
                    req = job.request
                    # Get current position
                    if hasattr(self.arm_controller, 'current_position'):
                        current_pos = self.arm_controller.current_position
                    else:
                        # Get from state if available
                        state = self.arm_controller.get_current_state()
                        ee_pos = state.get('ee_position', {})
                        current_pos = [ee_pos.get('x', 0.25), ee_pos.get('y', 0.0), ee_pos.get('z', 0.2)]
                    
                    target_pos = [
                        current_pos[0] + req['delta_x'],
                        current_pos[1] + req['delta_y'],
                        current_pos[2] + req['delta_z']
                    ]
    ...
    async def _execute_dry_run(self, job: Job):
    ...
    async def start(self):
        ...
        logger.info(f"Trajectory bridge server running on port {self.port}")
